...
export function HomePage() {
  ...
  const ratingLevel =
    (user?.rating_score ?? 0) >= 100 ? { color: 'text-primary', label: t('rating.elite') } :
    (user?.rating_score ?? 0) >= 70 ? { color: 'text-success', label: t('rating.advanced') } :
    (user?.rating_score ?? 0) >= 30 ? { color: 'text-accent', label: t('rating.active') } :
    { color: 'text-error', label: t('rating.beginner') };

  const newsTypeLabel = (type: string) =>
    type === 'news' ? t('news.type.news') :
    type === 'announcement' ? t('news.type.announcement') :
    t('news.type.result');
  ...
  return (
    <PageLayout>
      ...
      <section className="relative overflow-hidden rounded-2xl bg-gradient-hero border border-border p-8 md:p-12 mb-8">
        <div className="relative z-10">
          <div className="flex items-center gap-2 mb-4">
            <Badge variant="primary" size="md">{t('home.platform')}</Badge>
          </div>
          ...
          {(directions.length > 0 ? directions : [
            ...
            <Link key={dir.slug} to={`/directions/${dir.slug}`}>
              <Card className="text-center py-6 group">
                <div className="text-3xl mb-3 group-hover:scale-110 transition-transform"><Emoji>{dir.icon}</Emoji></div>
      ...
      {slots.length > 0 && (
        <section className="mb-8">
          <div className="flex items-center justify-between mb-5">
            <h2 className="section-title">{t('home.upcomingSlots')}</h2>
            ...
            {slots.map(slot => (
              <Card key={slot.id} hover={false} className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-text-primary">{dirName(slot.direction_id) || t('home.class')}</p>
                    <div className="flex items-center gap-3 mt-0.5">
                      <span className="text-sm text-text-secondary font-mono">{slot.start_time.slice(0, 5)} — {slot.end_time.slice(0, 5)}</span>
                      <span className="text-xs text-text-muted">{slot.current_count}/{slot.capacity} {t('home.spots')}</span>
                    </div>
                  </div>
                </div>
                <Link to="/schedule">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <section>
          <div className="flex items-center justify-between mb-5">
            ...
            <Link to="/news" className="text-sm text-primary hover:text-primary-light font-medium transition-colors">{t('home.allNewsLink')}</Link>
          </div>
          {news.length === 0 ? (
            <Card hover={false}>
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      {post.published_at && (
                        <span className="text-xs text-text-muted">
                          {new Date(post.published_at).toLocaleDateString(locale === 'ru' ? 'ru-RU' : 'en-US')}
                        </span>
                      )}
                    </div>
        ...
        <section>
          <div className="flex items-center justify-between mb-5">
            ...
            <Link to="/rating" className="text-sm text-primary hover:text-primary-light font-medium transition-colors">{t('home.leaderboardLink')}</Link>
          </div>
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-8">
        {[
          { to: '/materials', icon: <BookOpen size={22} />, color: 'primary', titleKey: 'home.materialsTitle' as const, descKey: 'home.materialsDesc' as const },
          { to: '/map', icon: <MapIcon size={22} />, color: 'secondary', titleKey: 'home.campusMap' as const, descKey: 'home.campusMapDesc' as const },
          { to: '/rewards', icon: <Star size={22} />, color: 'accent', titleKey: 'home.rewardsTitle' as const, descKey: 'home.rewardsDesc' as const },
          { to: '/schedule', icon: <Zap size={22} />, color: 'success', titleKey: 'home.scheduleTitle' as const, descKey: 'home.scheduleDesc' as const },
        ].map(link => (
          <Link key={link.to} to={link.to}>
            <Card className="group">
              ...
              <h3 className="font-semibold text-sm mb-0.5">{t(link.titleKey)}</h3>
              <p className="text-xs text-text-muted">{t(link.descKey)}</p>
            </Card>
          </Link>
        ))}
      </section>
